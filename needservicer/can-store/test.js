...
function initialize(products) {
    ...
    function fetchBlob(product) {
        ...
        fetch(url)
            ...
            .catch(err => console.error(`讀取錯誤:${err.message}`));
    }
    ...
    function showProduct(blob, product) {
        ...
        //每一個商品的標題開頭為大寫，其餘小寫
        heading.innerText = product.name.replace(product.name.charAt(0), product.name.charAt(0).toUpperCase());

        //添加商品價格
        para.innerText = "$" + product.price.toFixed(2);
